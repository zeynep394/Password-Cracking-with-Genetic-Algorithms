...
class Chromosome implements Comparable<Chromosome>{  // Kromozom nesnelerinin oluşturulacağı kromozom sınıfı
    ...
    public Chromosome(int matches,String[] chromosome){ // kromozomu oluşturmak için constructor metot

        this.matches=matches;
        this.chromosome=chromosome;

        //hashtable aynı veri tipini vermek için inti string olarak ata (tip dönüşümü)
    ...
    public ArrayList<String[]> create_chromosome(int passcode_length,int population_size/*,String[] chromosome*/){
        //static dersen sadece bu classın içinde erişilebiliri main classı obje oluştursan bile bu methoda erişemez
        ArrayList<String[]> population = new ArrayList<>();
        for( int i=0; i<population_size;i++){

            String[] chromosome =new String[passcode_length];
            for(int x=0; x<passcode_length;x++){
                //String chars = "AaBbCcÇçDdEeFfGgĞğHhIıİiJjKkLlMmNnOoÖöPpRrSsŞşTtUuÜüVvYyZz";
    ...
    {
        int compareage = ((Chromosome)comparestu).getMatches();//fitness_scores arrayListesini matches değerine göre sırala

        return compareage-this.matches; //descending order

    }

    public ArrayList<Chromosome> parentsList(ArrayList<Chromosome> fitness_scores){
        ArrayList<Chromosome> parentsList = new ArrayList<>(); //array de yapılabilirdi
    ...
    public Chromosome breed(Chromosome parent1, Chromosome parent2,ArrayList<Chromosome> parents ){
        ...
        String[] child= new String[passcode_length]; // yeni nesil için üretilecek olan çocuk kromozomu tutacak array
    ...
    public ArrayList<Chromosome> create_children(ArrayList<Chromosome> parentsList, ArrayList<String[]> population){
        ArrayList<Chromosome> children = new ArrayList<>(); // oluşturulan çocukların tutulacağı arraylist
    ...
    public static void main(String[] args) {
        ...
        for (int f = 0; f < döngüSay; f++) {
            ...
            while (true) {
                ...
                if (found == 1) { // 1 kez bulduysa onu görüntüleyecek(bulduğu ilk generation'u)
                    break;
                }
            ...
            population_size+=5; //populasyon boyutunun etkisini gözlemlemek için her turda populasyon boyutunu 5 artırdık
        }
